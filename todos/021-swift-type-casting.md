...
`throw`나 `do-catch`가 관여하지 않는다. [오류 처리](./100-swift-error-handling-forms.md)와는 별개의 메커니즘이다.
...
이 예제는 같은 파일 안에서 보내고 받으므로 사실상 확실하다. 하지만 **`NotificationCenter`는 누구나 같은 이름으로 알림을 보낼 수 있는 구조**다([NotificationCenter 문서](./109-notification-center.md) 참조). 다른 코드가 `"Course"`에 `String`을 넣어 보낼 수도 있다.
...
`URL(string:)!`과 같은 위험을 갖는다([AsyncImage 문서](./121-async-image.md)에서 다룬 크래시). **확신이 있어도 `as?` + `guard`가 낫다.**
...
[NS 접두사 문서](./116-ns-prefix-foundation-classes.md)에서 다룬 값 타입 ↔ 참조 타입 대응이다.
...
이 구분이 중요하다. [CGFloat 문서](./061-coregraphics-types-and-cgfloat.md)에서 다룬 `CGFloat(columns)`도 캐스팅이 아니라 이니셜라이저 호출이다.
...
`userInfo`가 `[AnyHashable: Any]`인 이유는 **무엇이든 담을 수 있어야** 하기 때문이다. [NotificationCenter 문서](./109-notification-center.md)에서 다룬 대로 `Codable` 같은 제약이 없다.

**`Any`는 편리하지만 타입 안전성을 포기하는 것이다.** 꺼낼 때마다 캐스팅해야 하고, 컴파일러가 검증해 주지 않는다. 가능하면 구체 타입이나 제네릭을 쓰는 편이 낫다([제네릭 문서](./030-swift-generics.md) 참조).
