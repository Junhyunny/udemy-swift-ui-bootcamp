...
`Body`가 `associatedtype`이다. 즉 **구현하는 타입마다 `body`의 타입이 다르다.** [typealias와 associatedtype 문서](./032-typealias-and-associated-type.md)에서 다룬 자리 표시자다.
...
modifier를 하나 붙일 때마다 `ModifiedContent`가 중첩되어 타입이 길어진다. [ViewModifier 문서](./042-view-modifier-protocol.md)에서 본 구조다.
...
[클로저와 view builder 문서](./040-closures-and-view-builders.md)와 [ViewBuilder vs View 구조체](./041-viewbuilder-vs-view-struct.md)에서 다룬 내용이다.
...
`trigger: some Equatable`은 "`Equatable`을 만족하는 어떤 타입"이다. [phaseAnimator 문서](./091-phase-animator-parameters-and-phase-types.md) 참조.
...
- [typealias와 associatedtype](./032-typealias-and-associated-type.md)
- [protocol 요구사항과 style 프로토콜](./036-protocol-requirements-and-style-protocols.md)
...
- [클로저와 view builder](./040-closures-and-view-builders.md)
- [ViewBuilder 함수 vs View 구조체](./041-viewbuilder-vs-view-struct.md)
...
- [NavigationPath와 타입 배열](./075-navigation-path-and-typed-array.md) — path가 타입 소거를 쓰는 이유
...
Xcode에서 `body`에 ⌥클릭하면 Quick Help가 `some View`라고만 보여 준다. 하지만 `print(type(of: body))`를 실행하면 실제 중첩 타입이 드러난다. [메타타입과 `.self`](./035-metatype-and-self.md)에서 다룬 `type(of:)`다.
