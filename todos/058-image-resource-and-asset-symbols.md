...
asset catalog의 구조와 배율 슬롯은 [별도 문서](./055-asset-catalog-universal-scale.md)에 정리했다. 이 문서는 **코드에서 이미지를 가리키는 방법**의 변천을 다룬다.
...
값 타입이다. 복사되며 참조 카운팅 대상이 아니다. [struct와 class](./019-struct-vs-class.md)의 구분이 그대로 적용된다.
...
그래서 예제가 `.pic1`이라고만 써도 컴파일된다. `ImageResource` 타입의 정적 프로퍼티로 만들어져 있기 때문에 [타입 추론에 의한 축약](./015-static-type-properties-and-implicit-init.md)이 가능하다.
...
이것이 `PhaseAnimator`의 phases로 넘어갈 수 있는 이유이기도 하다. `ImageResource`가 `Equatable`을 만족하므로 `where Phase : Equatable` 제약을 통과한다. [phase 타입 이야기](./091-phase-animator-parameters-and-phase-types.md) 참조.
