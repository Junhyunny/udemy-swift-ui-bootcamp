...
암시적 애니메이션과 `withAnimation`의 차이는 [별도 문서](./090-implicit-vs-explicit-animation.md)에, `animation(_:value:)`의 `value`가 필요한 이유는 [여기](./089-animation-value-trigger.md)에 정리했다. 이 문서는 **`phaseAnimator`** 에 집중한다.
...
이 성질은 [`animation(_:value:)`의 `value`](./089-animation-value-trigger.md)와 같은 발상이다. 값이 무엇인지가 아니라 **변했는지**가 신호다.
...
이름 그대로 자리 표시자다. `.phaseAnimator`가 붙은 원래 뷰(예제에서는 `VStack`)를 대신하고, 여기에 효과를 얹으면 실제 뷰에 적용된다. [`ViewModifier`의 `content`](./042-view-modifier-protocol.md)와 같은 개념이다.
...
명시적 애니메이션이다. [비교 문서](./090-implicit-vs-explicit-animation.md) 참조.
...
연관값 없는 `enum`은 `Equatable`이 자동 합성되므로 바로 쓸 수 있다. [enum과 Hashable 문서](./028-enum-hashable-conformance.md)에서 다룬 합성 규칙이 그대로 적용된다.
