...
`static` 저장/계산 프로퍼티의 차이는 [별도 문서](./016-static-stored-vs-computed-property.md)에 정리했다. 이 문서는 **지역 계산 프로퍼티와 즉시 실행 클로저**를 다룬다.
...
**프로토콜이 아니라 클래스**다. `Formatter`를 상속하고, `Formatter`는 `NSObject`를 상속한다. [NS 접두사 문서](./116-ns-prefix-foundation-classes.md)에서 다룬 Objective-C 계열 타입이다.
...
**읽기 전용이면 `get`을 생략할 수 있다.** [static 프로퍼티 문서](./016-static-stored-vs-computed-property.md)에서 다룬 `static var sample: [Course] { ... }`와 같은 문법이다.
...
**`return`도 생략할 수 있다.** 단일 표현식이면 [클로저의 암시적 반환](./007-closure-shorthand-argument-names.md)과 같은 규칙이 적용된다.
...
읽기 전용이라도 `let`을 쓸 수 없다. 값이 고정된 것이 아니라 **매번 계산**되기 때문이다. [static 프로퍼티 문서](./016-static-stored-vs-computed-property.md)에서 "계산 프로퍼티가 항상 `var`인 이유"로 다룬 내용이다.
...
- **`Decodable`이 무시한다.** 저장 프로퍼티가 아니므로 JSON에 `id` 키가 없어도 된다. [Codable 문서](./117-codable-and-codingkey.md)에서 `CodingKeys`로 `id`를 제외해야 했던 문제가 애초에 생기지 않는다
- **신원이 안정적이다.** `objectID`에서 파생되므로 같은 데이터면 항상 같은 `id`다. [`let id = UUID()`의 함정](./054-duplicate-id-in-list.md)을 피한다
...
**앞 두 글자는 국가 코드이고 로케일의 첫 부분은 언어 코드**다. 서로 다르다. [국기 이모지 문서](./138-flag-emoji-from-unicode-scalars.md)에서 다룬 것과 같은 관례에 기댄 코드인데, 로케일에서는 통하지 않는다.
...
**한 줄이면 끝난다.** [Foundation 문서](./115-foundation-framework.md)에서 다룬 `FormatStyle` API다. 캐싱을 신경 쓸 필요도 없고, 내부적으로 최적화되어 있다.
