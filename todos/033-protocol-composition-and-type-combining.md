...
`typealias`는 **새 타입을 만들지 않는다.** 그냥 긴 이름의 짧은 별칭이다. [`typealias`와 associated type 문서](./032-typealias-and-associated-type.md), [`Codable`과 `CodingKeys` 문서](./117-codable-and-codingkey.md)를 참고한다.
...
[Swift 제네릭 문서](./030-swift-generics.md)와 이어진다.
...
`RawRepresentable`은 `RawValue`라는 연관 타입을 갖는다. "raw value가 `String`인 `RawRepresentable`"은 `&` 문법으로 표현할 수 없고 `where` 절이 필요하다. 자세한 내용은 [`where` 키워드 문서](./031-where-clause-usages.md)에 정리되어 있다.
...
`View`를 반환할 때 쓰는 `some View`가 바로 이 `some`이다. [`some` 키워드 문서](./034-some-keyword-opaque-types.md)에 정리되어 있다.
...
남의 타입(표준 라이브러리, 서드파티)에도 붙일 수 있다는 것이 Swift 프로토콜의 강력한 점이다. [`extension` 키워드 문서](./014-extension-keyword.md)와 이어진다.
...
[`enum` raw value 문서](./011-enum-raw-values.md), [`enum`의 `Hashable` 문서](./028-enum-hashable-conformance.md), [`CaseIterable`과 `Sequence` 문서](./029-caseiterable-and-sequence.md)를 함께 보면 좋다.
...
`Array(_:)`는 어떤 `Collection`이든 받으므로 `AllCases`가 무엇이든 동작한다. [Swift의 형변환 문서](./021-swift-type-casting.md)와 이어진다.
