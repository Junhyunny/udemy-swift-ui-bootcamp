...
`==`도 값 비교가 아니라 식별자 비교다. `Todo`는 class이므로 애초에 값 타입 의미의 동등성이 아니다. 이 부분은 [`struct`와 `class`](./019-struct-vs-class.md), [`@Model`은 왜 class인가](./126-swiftdata-model-class-and-final.md)와 이어진다.
...
이게 가능한 이유는 [매크로 확장 결과](./037-swift-macros-and-build-pipeline.md)에 있다. `var title: String`이 저장소를 읽고 쓰는 accessor로 바뀌기 때문에, 프로퍼티 대입이 곧 저장소 변경 추적이 된다.
...
자세한 내용은 [SwiftData 동시성과 context 격리](./132-swiftdata-concurrency-and-context-isolation.md)에서 다룬다.
