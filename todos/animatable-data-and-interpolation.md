...
중첩이 사라지고 `.0`, `.1`로 접근한다. `@Animatable` 매크로가 합성하는 것도 바로 이 `AnimatableValues`다. [`@Animatable` 매크로 문서](./animatable-macro.md)에서 실제 확장 결과를 볼 수 있다.
...
[암시적·명시적 애니메이션 문서](./implicit-vs-explicit-animation.md), [`animation(_:value:)`의 `value` 문서](./animation-value-trigger.md)와 이어진다.
