...
어떤 API에 `value`를 주고 어떤 API에 `$value`를 주는지는 [프로퍼티 래퍼의 `$` 사용 기준](./044-property-wrapper-dollar-sign.md)에 정리돼 있다. 이 문서는 **`@State`가 프로퍼티를 어떤 모양으로 바꿔 놓는지**와 **`$`가 통하지 않는 상황**을 다룬다.
