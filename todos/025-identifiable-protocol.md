...
`ForEach`에서 `id:`를 직접 넘기는 쪽 이야기는 [`ForEach`의 `id`와 `\.self` key path](./053-foreach-id-and-identity-keypath.md)에 정리돼 있다. 이 문서는 **타입 자체에 identity를 부여하는** `Identifiable` 쪽을 다룬다.
