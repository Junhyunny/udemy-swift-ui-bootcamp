...
메타타입과 `.self`의 언어 차원 의미는 [별도 문서](./035-metatype-and-self.md)에 정리했다. 이 문서는 **네비게이션에서 그것이 어떻게 쓰이는가**를 다룬다.
...
②가 중요하다. 링크를 탭하는 것과 화면이 그려지는 것 사이에 `path`라는 상태가 놓여 있다. 그래서 **링크를 탭하지 않고 코드로 `path.append(...)`를 해도 똑같이 화면이 이동한다.** [path 문서](./075-navigation-path-and-typed-array.md)에서 다룬 프로그래밍 방식 네비게이션이 이 구조 덕분에 가능하다.
...
이 패턴은 SwiftUI 곳곳에 있다. [`preference(key:)`](./067-preference-key-and-onpreferencechange.md)의 `SizePreferenceKey.self`, `onGeometryChange(for: CGSize.self)`, `Codable`의 `decode(User.self, forKey:)`가 모두 같은 발상이다. **"어떤 타입인지"가 곧 정보인 자리**다.
...
목록이 길면 성능 차이가 크다. [`NavigationView`와의 비교 문서](./073-navigation-stack-vs-navigation-view.md)에서 다룬 개선점이 이 부분이다.
...
`where D : Hashable` 제약이다. `String`과 `Color`는 이미 만족한다. 직접 만든 타입은 준수를 선언해야 한다. [enum과 Hashable](./028-enum-hashable-conformance.md) 참조.
