...
`try`/`try?`/`do-catch`의 선택 기준은 [별도 문서](./100-swift-error-handling-forms.md)에, `async/await` 실행 모델은 [여기](./101-swift-async-await-model.md)에 정리했다. 이 문서는 **`async`와 `throws`의 조합**과 **에러 타입 만들기**를 다룬다.
...
`await`이 없는 곳에서는 절대 멈추지 않는다. [async/await 모델 문서](./101-swift-async-await-model.md)에서 다룬 내용이다.
...
`do-catch`가 사라져 코드도 짧아진다. 오류 처리는 UI를 아는 뷰에서 하는 것이 맞다. [.task 문서](./103-task-modifier-and-async-lifecycle.md)에 관련 내용이 있다.
...
에러 종류와 함께 **어떤 상태 코드였는지**를 전달한다. 이게 `enum` 에러의 핵심 강점이다. [raw value와 연관값 구분](./011-enum-raw-values.md) 참조.
...
[Codable 문서](./117-codable-and-codingkey.md)에서 `DecodingError`의 종류를 다뤘다.
