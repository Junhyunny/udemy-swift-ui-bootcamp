...
`Array`는 제네릭 타입이고 `[Post]`는 그것의 **문법적 설탕(syntactic sugar)** 이다. [제네릭 문서](./030-swift-generics.md)에서 다룬 대로 `Dictionary`, `Optional`도 같은 축약을 갖는다.
...
초기값이 있으므로 별도 `init`이 필요 없다. **`@Observable`은 초기화 방식에 관여하지 않는다** — 이 프로퍼티가 관찰 대상이 되는 것과 초기화 표기는 무관하다. [Observation 문서](./050-observation-framework-and-observable.md) 참조.
