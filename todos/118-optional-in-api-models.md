...
`Codable`과 `CodingKeys`는 [별도 문서](./117-codable-and-codingkey.md)에 정리했다. 이 문서는 **모델 설계에서 옵셔널을 언제 쓸지**를 다룬다.
...
- 실패 원인을 [구체적인 오류로](./102-async-throws-and-custom-errors.md) 표현할 수 있다
...
[Hashable과 신원 문제](./026-hashable-id-and-collisions.md)에서 다룬 함정이다. `Equatable`을 채택했지만 `id`가 `CodingKeys`에 없어 디코딩마다 새 UUID가 생긴다. 같은 응답을 두 번 받으면 `==`가 `false`다.
...
[`replaceError(with:)`](./113-combine-operators.md)와 결합해 **모든 실패가 빈 데이터로 뭉개진다.** 옵셔널 설계와 오류 처리가 함께 문제를 만드는 사례다.
