...
**서버가 준 JSON 키가 Swift 예약어인 경우**가 백틱이 정말 필요한 상황이다. 다만 그때는 [`CodingKeys`로 매핑](./117-codable-and-codingkey.md)하는 편이 더 깔끔하다.
...
관련해서 `Self`(대문자)와 `self`(소문자)의 차이는 [메타타입 문서](./035-metatype-and-self.md)에서 다뤘다.
