...
오류 타입은 같지만 값에 따라 다르게 처리할 때 쓴다. [Swift 오류 처리 문서](./100-swift-error-handling-forms.md)와 이어진다.
...
제네릭 쪽 `where`는 [Swift 제네릭 문서](./030-swift-generics.md)에 더 정리되어 있다.
...
남겨 두면 제약이 선언부에 드러나 읽기 좋고, 지우면 중복이 사라진다. 어느 쪽도 틀리지 않는다. [프로토콜 합성 문서](./033-protocol-composition-and-type-combining.md)에서 이어서 다룬다.
