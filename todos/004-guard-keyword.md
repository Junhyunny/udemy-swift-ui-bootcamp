...
블록 안에서 `urlString`을 쓰면 **새로 만든 비옵셔널 상수**를 가리키고, 원래 옵셔널은 가려진다(shadowed). 이것이 [`if` 조건 문서](./003-if-conditions-and-optional-binding.md)에서 다룬 **그늘짐(shadowing)** 이다.
...
[`[weak self]` 문서](./023-weak-self-and-deinit.md)에서 다룬 형태다. 문법은 같지만 "옵셔널 `self`를 비옵셔널로"라는 목적이 뚜렷하다.
...
이 예제는 `.link` 타입이 항상 유효해서 실패할 일이 사실상 없으므로 `try?`로 충분하다. 선택 기준은 [Swift의 오류 처리 방식들](./100-swift-error-handling-forms.md)에 정리했다.
