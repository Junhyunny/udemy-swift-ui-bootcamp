...
애니메이션 API 세 가지 비교는 [별도 문서](./092-animation-api-comparison.md)에, 암시적·명시적 구분은 [여기](./090-implicit-vs-explicit-animation.md)에 정리했다. 이 문서는 **`delay`의 동작 원리**를 다룬다.
...
[애니메이션 API 문서](./092-animation-api-comparison.md)에서 다룬 다른 수식자들과 같은 방식이다.
...
**이들은 모두 `Animation` 값을 변형해 새 값을 만든다.** [값 타입의 일반적 패턴](./019-struct-vs-class.md)이다.
...
**`withAnimation`은 "이 상태 변경에 이 애니메이션을 붙여라"를 트랜잭션에 심는다.** [명시적 애니메이션 문서](./090-implicit-vs-explicit-animation.md)에서 다룬 내용이다.
...
`delay` 값을 하드코딩으로 맞추면 하나를 바꿀 때 나머지도 조정해야 한다. 단계가 많으면 [`phaseAnimator`](./091-phase-animator-parameters-and-phase-types.md)가 더 적합하다.
...
[`@Environment` 문서](./048-environment-property-wrapper.md)에서 다룬 접근성 값이다. `nil`을 넘기면 애니메이션 없이 즉시 반영된다.
...
[`.task`](./103-task-modifier-and-async-lifecycle.md)를 쓰면 뷰가 사라질 때 자동 취소되므로 `DispatchQueue`보다 안전하다.
