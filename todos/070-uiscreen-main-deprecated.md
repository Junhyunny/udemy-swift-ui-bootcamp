...
`UI` 접두사와 UIKit 연동은 [별도 문서](./087-uiviewrepresentable-and-uikit-bridge.md)에, `GeometryReader`는 [여기](./065-geometry-reader-use-cases.md)에 정리했다. 이 문서는 **화면 크기를 얻는 최신 방법**을 다룬다.
...
[GeometryReader 문서](./065-geometry-reader-use-cases.md)에서 다룬 대로, 이것이 "부모가 준 공간"을 아는 표준 수단이다.
...
정확한 픽셀 대신 compact/regular 구분이 필요하면 이쪽이 맞다. [`@Environment` 문서](./048-environment-property-wrapper.md) 참조.
...
SwiftUI에서는 [`@Environment(\.displayScale)`](./048-environment-property-wrapper.md)이 대응한다.
...
**오프셋 계산을 뷰로 옮기고 뷰모델은 "숨김 여부"만 갖는다.** 뷰모델이 화면 크기를 알 필요가 없어지고, [UIKit 의존도 사라진다](./049-state-wrapper-decision-guide.md).
...
**뷰모델에는 타이머 상태(`time`, `selectedTime`)만 남는다.** [상태 관리 문서](./049-state-wrapper-decision-guide.md)에서 다룬 "뷰 레이아웃 값은 뷰가 소유한다"는 원칙이다.
