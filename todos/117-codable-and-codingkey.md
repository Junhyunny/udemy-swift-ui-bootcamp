...
프로토콜이 아니라 **두 프로토콜을 묶은 별칭**이다. [typealias 문서](./032-typealias-and-associated-type.md)에서 다룬 "여러 프로토콜 묶기" 용법의 표준 라이브러리 사례다.
...
`News.self`처럼 타입을 넘기는 것도 [메타타입](./035-metatype-and-self.md)의 용법이다.

주요 용도를 정리하면 이렇다.

- **네트워크 응답 파싱** — 이 예제
- **로컬 저장** — `UserDefaults`, 파일에 JSON으로 저장
- **네비게이션 상태 복원** — [`NavigationPath.codable`](./075-navigation-path-and-typed-array.md)이 `Codable`을 요구한다
- **앱 간 데이터 전달**
...
여기서 `String`은 **상속이 아니라 raw value 타입 지정**이다. 자세한 내용은 [enum raw value 문서](./011-enum-raw-values.md)에 정리했다.
...
`url`을 `id`로 쓰면 **신원이 안정적**이라는 이점도 생긴다. `let id = UUID()`는 디코딩할 때마다 새 값이 되어, 같은 기사를 두 번 받으면 다른 것으로 취급된다. [`Hashable`과 신원 문제](./026-hashable-id-and-collisions.md)와 [static var 문제](./016-static-stored-vs-computed-property.md)에서 다룬 것과 같은 함정이다.
...
[커스텀 에러 문서](./102-async-throws-and-custom-errors.md)에서 이어서 다룬다.
