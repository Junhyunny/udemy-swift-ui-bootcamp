...
`NS` 접두사의 유래는 [별도 문서](./116-ns-prefix-foundation-classes.md)에 정리했다. 이 문서는 **Foundation 프레임워크의 구성**을 다룬다.
...
`Date`는 **시점(point in time)** 을 나타내는 값이다. 표시 형식은 `Calendar`, `TimeZone`, `Locale`에 따라 달라지므로 별도로 계산한다. [`@Environment`의 `\.locale`, `\.calendar`](./048-environment-property-wrapper.md)가 이것과 연결된다.
...
`UUID`는 충돌 가능성이 사실상 없는 128비트 식별자다. 다만 **호출할 때마다 새 값이 생성**되므로 주의가 필요하다. [static 프로퍼티 문서](./016-static-stored-vs-computed-property.md)에서 다룬 함정이고, 이 프로젝트도 같은 구조다.
...
[id 중복 문서](./054-duplicate-id-in-list.md)에서 이어서 다룬다.

**JSON 인코딩·디코딩**

[Codable 문서](./117-codable-and-codingkey.md)에서 다룬 `JSONDecoder`도 Foundation이다.

**네트워킹**

`URLSession`, `URLRequest` 모두 Foundation이다. [chapter-61의 네트워킹 코드](./030-swift-generics.md)가 이것을 썼다.
