...
만약 레이블 없이 `subscript(_ index: Int) -> Element?`로 정의했다면, 기존 것과 파라미터가 같고 반환 타입만 달라 **호출할 때마다 모호해진다.** 레이블은 멋 부리기가 아니라 **충돌 회피 장치**다. [Swift의 함수 오버로딩 문서](./swift-function-overloading.md)와 이어진다.
...
[`static` 타입 프로퍼티 문서](./static-type-properties-and-implicit-init.md), [`enum` raw value 문서](./enum-raw-values.md)와 함께 보면 좋다.
...
`@Observable`이나 `Binding`이 `$viewModel.currentIndex`처럼 동작하는 것도 내부적으로는 `subscript(dynamicMember:)`([SE-0252](https://github.com/swiftlang/swift-evolution/blob/main/proposals/0252-keypath-dynamic-member-lookup.md))라는 제네릭 subscript다. [`@State`와 `_viewModel` 문서](./state-property-wrapper-backing-storage.md)에서 이어서 다룬다.
