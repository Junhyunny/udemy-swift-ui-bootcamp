...
`some` 키워드와 opaque type은 [별도 문서](./034-some-keyword-opaque-types.md)에, `associatedtype`은 [여기](./032-typealias-and-associated-type.md)에 정리했다. 이 문서는 **제네릭 자체**를 다룬다.
...
`T.Type`은 **타입 자체를 받는 파라미터**다. `News.self`를 넘기면 `T`가 `News`로 결정된다. [메타타입과 `.self`](./035-metatype-and-self.md)에서 다룬 개념이고, `navigationDestination(for:)`이나 `decode(_:from:)`도 같은 방식이다.
...
`Element`가 `Equatable`일 때만 이 메서드가 생긴다. [extension 문서](./014-extension-keyword.md)에서 다룬 조건부 확장이다.
...
`any Decodable`을 돌려줬다면 `as? News` 캐스팅이 필요했다. [some과 any 비교](./034-some-keyword-opaque-types.md) 참조.
