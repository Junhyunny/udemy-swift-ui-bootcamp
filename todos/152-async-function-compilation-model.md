...
| **런타임** | 실제로 누가 실행하는가 — Task, Job, Executor, 스레드 풀 | [151](./151-concurrency-runtime-architecture.md) |
| **컴파일** | `async` 함수는 어떤 코드로 바뀌는가 | **이 문서** |

세 층은 같은 것을 다른 배율로 보는 것이다. 151이 "잡(Job)이 실행자에 들어간다"고 말할 때, **그 잡이 어떻게 만들어지는지**가 여기에 있다.
...
컴파일러는 `await`를 경계로 함수를 쪼갠다. 151에서 말한 **잡(Job)** 이 이 토막이다.
...
**함수 하나가 한 스레드에서 쭉 도는 것이 아니라, 토막들이 각각 스케줄링된다.** [151 문서](./151-concurrency-runtime-architecture.md) 7부의 순서도가 이 토막 단위로 일어나는 일이다.
...
- **`await`를 가로질러 살아남아야 하는 값만 프레임에 들어간다.** 그래서 크기가 커진다. [143 문서](./143-sync-vs-async-and-suspension.md)의 "대기 중 상태는 스레드 스택이 아니라 별도 저장소"가 바로 이 프레임이다
