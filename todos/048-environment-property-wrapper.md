...
`@State`, `@Binding`과 나란히 두면 성격이 분명해진다. 프로퍼티 래퍼 일반과 `$` 문법은 [프로퍼티 래퍼의 `$` 사용 기준](./044-property-wrapper-dollar-sign.md)에 정리했다.
...
`\.layoutDirection`은 [좌표 공간 문서](./066-coordinate-space-local-global-named.md)에서 다룬 RTL 대응과 이어진다.
...
`\.scenePhase`는 [생명주기 문서](./051-view-lifecycle-hooks.md)에서 다룬 앱 상태 전환과 직결된다.
...
`\.symbolRenderingMode`([관련 문서](./057-symbol-rendering-mode.md)), `\.redactionReasons`, `\.isScrollEnabled`, `\.widgetFamily` 등.
...
`DestView`처럼 **재사용되는 화면**은 자기가 어떤 스택에 올라와 있는지 알 필요가 없다. `dismiss()`만 호출하면 시스템이 알아서 처리한다. 반대로 "루트까지 한 번에" 같은 조작은 `path = []`가 필요하다. [`NavigationPath`와 타입 배열](./075-navigation-path-and-typed-array.md) 참조.
...
앱이 백그라운드에서 [`Timer`가 멈추는 문제](./110-timer-publisher-and-onreceive.md)를 보정하는 용도이고, 시뮬레이터에서 제외한 이유는 [별도 문서](./124-simulator-vs-device-behavior.md)에 정리했다.
...
**아래로만 흐르고, 더 안쪽에서 덮어쓸 수 있다.** [PreferenceKey](./067-preference-key-and-onpreferencechange.md)가 자식→조상인 것과 정확히 반대 방향이다.
...
`extension`의 적용 범위와 파일 관리 이야기는 [extension 문서](./014-extension-keyword.md)에 정리했다.
