...
- `Sendable`을 **일부러 사용 불가로 표시**한다. 이 한 줄이 SwiftData 동시성 규칙의 근거다. [SwiftData 동시성 문서](./132-swiftdata-concurrency-and-context-isolation.md)에서 이어서 다룬다.
...
property wrapper는 매크로가 아니지만 컴파일러가 저장 프로퍼티와 `wrappedValue` 접근으로 바꿔 주므로 "코드가 생기는 것처럼" 보인다. [프로퍼티 래퍼의 `$` 사용 기준](./044-property-wrapper-dollar-sign.md)과 함께 보면 구분이 쉽다.
