...
앞의 둘은 이미 [`@State`를 붙이면 타입이 바뀌는가 문서](./043-state-wrapper-type-and-binding.md)와 [프로퍼티 래퍼의 `$` 문서](./044-property-wrapper-dollar-sign.md)에서 다뤘다. **이 문서는 세 번째, `_` 를 다룬다.**
...
제안서는 **이 저장 프로퍼티가 항상 `private`**이라고도 못박는다. 즉 `_viewModel`은 `private`을 뚫는 이름이 아니라, **처음부터 `private`인 이름**이다. 다른 타입에서 `otherView._viewModel`을 쓰면 컴파일 오류가 난다. [Swift 접근 제어 문서](./017-access-control.md)와 이어진다.
...
[`.task` 문서](./103-task-modifier-and-async-lifecycle.md)와 이어진다.
...
`@Observable`이 나오기 전에는 `@StateObject`를 썼다. 지금 코드의 `OnboardingViewModel`은 `@Observable`이 붙은 `final class`이므로 `@State`가 맞다. [`Observation` 모듈과 `@Observable` 문서](./050-observation-framework-and-observable.md), [상태 관리 래퍼 총정리](./049-state-wrapper-decision-guide.md)를 함께 본다.
...
- 이 `viewModel`은 화면이 몇 번 다시 그려져도 **같은 인스턴스**로 유지되고, `$viewModel.currentIndex`로 바인딩을 꺼내 `scrollPosition`에 넘긴다. 그 부분은 [`scrollPosition(id:)` 문서](./081-scroll-position-binding.md)에서 이어서 다룬다.
...
프로퍼티 래퍼에서 매크로로 구현이 바뀐다는 뜻이다. 사용하는 문법(`@State private var x`)은 그대로지만, 생성되는 코드의 형태는 달라질 수 있다. 지금 단계에서는 **"`_` 접두 이름이 컴파일러 생성물"이라는 원리만 잡아 두면 충분**하다. [Swift macro 문서](./037-swift-macros-and-build-pipeline.md)와 이어진다.
