...
개별 문서가 이미 있다. [암시적 애니메이션과 `withAnimation`의 차이](./090-implicit-vs-explicit-animation.md), [`animation(_:value:)`의 `value`가 필요한 이유](./089-animation-value-trigger.md), [`phaseAnimator`의 파라미터](./091-phase-animator-parameters-and-phase-types.md). 이 문서는 **셋을 나란히 놓고 언제 무엇을 고를지**를 정리한다.
...
`value:`가 왜 필요한지는 [별도 문서](./089-animation-value-trigger.md)에 정리했다.
...
- 접근성 설정을 존중한다 — [`\.accessibilityReduceMotion`](./048-environment-property-wrapper.md)
