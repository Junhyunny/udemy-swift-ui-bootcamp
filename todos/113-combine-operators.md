...
구독 수명 관리는 [별도 문서](./112-combine-cancellable-and-store.md)에 정리했다. 이 문서는 **파이프라인 중간의 연산자들**을 다룬다.
...
chapter-61의 `catch {}`와 같은 문제다. [오류 처리 문서](./100-swift-error-handling-forms.md)에서 다룬 내용이다.
...
`map(\.data)`는 그중 `data`만 꺼낸다. `\.data`는 [key path](./053-foreach-id-and-identity-keypath.md)이고, `map { $0.data }`와 같다.
...
[메타타입 문서](./035-metatype-and-self.md)에서 다룬 `.self`다.
...
연산자를 하나 추가할 때마다 타입이 중첩된다. [`some View`와 `ModifiedContent`](./034-some-keyword-opaque-types.md)에서 본 것과 같은 구조다. [AnyPublisher 문서](./111-combine.md)에서 이어서 다룬다.
