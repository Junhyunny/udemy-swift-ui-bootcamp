...
값 타입과 참조 타입의 선택 기준은 [struct와 class](./019-struct-vs-class.md)에 정리했다.
...
다만 **공식 문서가 이 저장소를 "영역"으로 규정하지는 않는다.** 컴파일 시점에 결정 가능한 대부분의 메타데이터는 실행 파일에 정적으로 박히고, 제네릭처럼 런타임에 필요한 경우에만 동적으로 만들어진다. `.self`의 언어 차원 의미는 [메타타입과 `.self`](./035-metatype-and-self.md)에 정리했다.
