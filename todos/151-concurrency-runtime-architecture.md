# Swift 동시성 런타임 아키텍처 — JavaScript 이벤트 루프와 비교하며

JavaScript에는 "이벤트 루프 + 콜스택 + 태스크 큐"라는 **한 장으로 그릴 수 있는 모델**이 있다. Swift에도 있다. 다만 이름이 널리 알려지지 않았을 뿐이고, 구조는 **작업(Task) → 잡(Job) → 실행자(Executor) → 협력 스레드 풀** 네 층이다.

[실행 모델 문서](./101-swift-async-await-model.md)가 "JS와 의미가 어떻게 다른가"를 다룬다면, 이 문서는 **그 아래에서 실제로 무엇이 돌아가는가**를 그린다. [로드맵](./142-swift-concurrency-roadmap.md)의 1·3단계와 함께 읽으면 좋다.
...
```text
   ...
   │    부모 ─┬─ 자식1                         │
   │          └─ 자식2                         │
   │  · 취소·우선순위·태스크 로컬이 트리를 타고 전파  │
   └────────────────────┬────────────────────┘
                        │ await 를 경계로 Job 단위로 쪼개짐
                        ▼
   ┌─────────────────────────────────────────┐
   │  Job (실행 가능한 작업 조각)                │
   │  · await 사이의 코드 한 토막                │
   ...
   │  Executor (실행자)                                        │
   │                                                          │
   │   MainActor        actor A         actor B     비격리      │
   │   직렬 실행자        직렬 실행자      직렬 실행자    (전역 동시) │
   │      │                │                │           │      │
   └──────┼────────────────┼────────────────┼───────────┼──────┘
          │                └────────┬───────┘           │
          ▼                         ▼                    ▼
   ┌─────────────┐        ┌──────────────────────────────────┐
   │  메인 스레드   │        │  협력 스레드 풀 (cooperative pool)  │
   │  (런루프)     │        │  스레드 수 ≤ 코어 수 — 늘어나지 않는다 │
   └─────────────┘        └──────────────────────────────────┘
```

**핵심은 가운데 층이다.** "이 코드가 어느 실행자에 들어가는가"를 정하는 것이 5~6단계에서 배운 **액터 격리**이고, 실행자가 곧 실행 위치를 결정한다.

## 2부 — JavaScript와 나란히 놓기
...
작업의 단위이자 **취소·우선순위·태스크 로컬 값이 흐르는 경로**다. 부모-자식 관계를 이루며, 4단계에서 확인한 취소 전파가 이 트리를 타고 간다.

### Job — 실행 가능한 조각

async 함수는 컴파일 시 **`await`를 경계로 여러 토막으로 쪼개진다.** 각 토막이 잡이다.

> An `ExecutorJob` represents "a chunk of work that an executor should execute."

잡은 우선순위를 들고 다니며, **한 번만 실행된다.** "async 함수 하나가 통째로 한 스레드에서 도는 것"이 아니라 "토막들이 각각 스케줄링된다"는 점이 중요하다. 그래서 `await` 앞뒤로 스레드가 달라질 수 있다.

### Executor — 잡을 실행하는 서비스

> The base `Executor` protocol is "a service that can execute jobs" with an `enqueue(_:)` method. Below that sits `SerialExecutor`, which "executes jobs one-by-one, and specifically, guarantees mutual exclusion between job executions."

두 종류를 구분하면 된다.

- **직렬 실행자(SerialExecutor)** — 한 번에 하나. 모든 `actor`와 `@MainActor`가 자기 것을 갖는다. **상호 배제를 제공하는 주체가 바로 이것**이다
- **전역 동시 실행자** — 비격리 코드가 들어가는 곳. 여러 개가 동시에 돈다

모든 actor는 `unownedExecutor` 프로퍼티를 가지며 컴파일러가 자동으로 만들어 준다. `MainActor`는 `sharedUnownedExecutor`로 자기 실행자를 노출한다. **5단계에서 배운 "actor가 직렬화한다"의 실체가 이 직렬 실행자다.**

### 협력 스레드 풀 — 스레드를 공급한다

전역 동시 실행자를 뒷받침하는 스레드 풀이다. **코어 수를 넘지 않는다.**
...
그래서 무거운 계산은 메인 액터 밖으로 보내고 결과만 돌려받는다. `@concurrent`가 그 표시이며, 측정 결과는 [139 문서](./139-actor-isolation-domains.md)의 3부에 있다.

## 7부 — `await` 한 줄에서 벌어지는 일
...
```text
  ① 잡 실행 중 (스레드 A 점유)
        ...
        ▼
  ② await 도달
        │  · 지역 변수와 재개 지점을 힙의 태스크 프레임에 저장
        │  · 스레드 A 를 즉시 반납   ← 여기가 JS 와 가장 다른 지점
        ▼
  ③ 스레드 A 는 다른 잡을 실행한다
        │
        ▼
  ④ 기다리던 일이 끝남 (네트워크 응답 등)
        │
        ▼
  ⑤ 재개 잡을 만들어 실행자에 enqueue
        │  · 어느 실행자? → 그 코드의 격리가 정한다
        ▼
  ⑥ 실행자가 잡을 꺼내 실행 (스레드 B 일 수 있다)
        │
        ▼
  ⑦ 저장해 둔 프레임을 복원해 그 줄부터 이어서 실행
```

세 가지를 기억하면 된다.

- **스레드는 `await`를 가로질러 유지되지 않는다.** 앞뒤 스레드가 같다고 가정한 코드를 쓰면 안 된다
- **중단된 상태는 스레드 스택이 아니라 힙**에 있다. 그래서 스레드를 놓고도 이어서 실행할 수 있다
- **재개 위치는 격리가 정한다.** 5~6단계 내용이 여기서 쓰인다

액터 재진입 문제도 이 그림으로 설명된다. ②에서 실행자를 놓는 순간 다른 잡이 그 actor에 들어올 수 있다. [147 문서](./147-actor-and-reentrancy.md)에서 잔액이 -100이 된 이유다.
...
| "블로킹해도 좀 느릴 뿐" | 풀이 고갈되면 **멈춘다** (5부 실측) |
