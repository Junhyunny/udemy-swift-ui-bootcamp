...
`NavigationStack`이 `NavigationView`를 어떻게 대체했는지는 [별도 문서](./073-navigation-stack-vs-navigation-view.md)에 정리했다. 이 문서는 **`path`를 무엇으로 관리할 것인가**를 다룬다.
...
`Hashable`뿐 아니라 **`Codable`도 요구한다.** 상태 저장·복원을 위해서다. [enum과 Hashable 문서](./028-enum-hashable-conformance.md)에서 다룬 자동 합성 덕분에 단순한 타입은 그냥 통과한다.
...
[`\.scenePhase`](./048-environment-property-wrapper.md)와 [`onChange`](./052-onchange-old-new-value.md)의 조합이다.
...
[chapter-51의 딥링크 예제](./085-deep-link-and-url-scheme.md)가 이 패턴이었다.
