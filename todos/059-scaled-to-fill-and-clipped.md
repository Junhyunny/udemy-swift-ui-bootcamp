...
프리뷰의 파란 테두리와 `Image` 크기 modifier의 기초는 [별도 문서](./056-image-layout-and-preview-bounds.md)에 있다. 이 문서는 **실제로 겪은 증상**의 원인을 파고든다.
...
modifier 순서가 결과를 바꾼다는 원칙은 [ViewModifier 문서](./042-view-modifier-protocol.md)에서도 다뤘다.
...
`.id(index)`가 이미지마다 별개의 뷰임을 알리고, `.transition(.opacity)`가 두 이미지를 겹쳐 페이드시킨다. 곡선도 스프링(`.snappy`)보다 `.easeInOut`이 자연스럽다. 세 API의 선택 기준은 [애니메이션 API 비교](./092-animation-api-comparison.md)에 정리했다.
