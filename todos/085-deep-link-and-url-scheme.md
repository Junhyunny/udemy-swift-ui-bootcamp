...
브라우저에서 앱을 찾아가는 **동작 원리와 스킴 충돌**은 [별도 문서](./086-url-scheme-resolution-and-conflicts.md)에 있다. 이 문서는 **무엇에 쓰는가**와 **설정 항목의 의미**를 다룬다.
...
[chapter-48의 `openURL`](./048-environment-property-wrapper.md)이 이 호출부에 해당한다. 즉 딥링크는 **보내는 쪽과 받는 쪽**이 있고, chapter-48은 보내는 쪽, chapter-51은 받는 쪽이다.
...
커스텀 스킴은 아무나 `devtechie`를 등록할 수 있지만, Universal Links는 도메인 소유자만 연결할 수 있다. 자세한 충돌 이야기는 [스킴 해석과 충돌 문서](./086-url-scheme-resolution-and-conflicts.md)에 있다.
...
**identifier가 고유해도 스킴 충돌을 막지 못한다.** 이름을 붙였을 뿐이지 소유권을 주장하는 것이 아니다. 실제 충돌 시 동작은 [스킴 해석과 충돌 문서](./086-url-scheme-resolution-and-conflicts.md)에서 다룬다.
