...
`x = 0`이 항상 왼쪽이 아니라 **leading**이라는 점이 중요하다. RTL 언어 환경에서는 오른쪽이 원점이 된다. 이 예제의 `Canvas` 정규화 좌표와 같은 방향 규칙을 쓰므로 [069-canvas-coordinate-space-and-normalization.md](./069-canvas-coordinate-space-and-normalization.md)와 함께 보면 좋다.
