...
이 성질은 문서 곳곳에서 이미 봤다. [`at:` 문법과 IndexSet](./008-argument-labels-and-indexset.md), [클로저의 trailing closure 규칙](./040-closures-and-view-builders.md)에서 다룬 argument label이 여기서도 작동한다.
...
비교하는 프로퍼티만 다르다. [key path](./053-foreach-id-and-identity-keypath.md)로 묶을 수 있다.
...
`getCountryBy`를 부를 때마다 배열 30개가 재생성된다. [ViewModel의 `emojiFlag`](./138-flag-emoji-from-unicode-scalars.md)가 목록의 모든 행에서 호출되므로, 행 개수 × 30개 생성이 된다.

`static let`으로 바꾸면 한 번만 만들어진다. [static var vs static let 문서](./016-static-stored-vs-computed-property.md)에서 다룬 문제다.
