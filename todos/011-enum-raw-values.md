...
`enum`이 `Hashable`을 채택하는 이야기는 [별도 문서](./028-enum-hashable-conformance.md)에 있다. 이 문서는 **콜론 뒤에 `String` 같은 타입이 오는 경우**를 다룬다.
...
[Swift의 타입 체계와 상속](./020-swift-type-system-and-inheritance.md)에서 다룬 대로, Swift에서 상속은 `class`에만 있다.
...
[Codable 문서](./117-codable-and-codingkey.md)에서 이어서 다룬다.
...
[에러 타입 문서](./102-async-throws-and-custom-errors.md)에서 이어서 다룬다.
