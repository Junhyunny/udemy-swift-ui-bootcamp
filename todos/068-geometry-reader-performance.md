...
용도와 활용 케이스는 [`GeometryReader`를 언제, 왜 쓰는가](./065-geometry-reader-use-cases.md)에 정리했다. 이 문서는 **성능 측면**만 파고든다.
...
두 번째가 특히 중요한 안전장치다. 자세한 내용은 [PreferenceKey 문서](./067-preference-key-and-onpreferencechange.md)에 있다.
...
바깥 `GeometryReader` 없이도 조상 기준 좌표를 얻는다. [좌표 공간 문서](./066-coordinate-space-local-global-named.md)에 정리했다.
