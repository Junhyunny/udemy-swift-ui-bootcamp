...
질문의 "CG 클래스들"이라는 표현을 정확히 하면, `CG` 타입은 두 부류로 갈린다. [`struct`와 `class`](./019-struct-vs-class.md)의 차이가 그대로 적용된다.
...
앞서 정리한 [좌표 공간 문서](./066-coordinate-space-local-global-named.md)의 `frame(in:)`이 돌려주는 것도 `CGRect`, `bounds(of:)`도 `CGRect?`다.
