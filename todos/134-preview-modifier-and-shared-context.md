...
매크로 확장 자체에 대한 배경은 [Swift macro와 빌드 파이프라인](./037-swift-macros-and-build-pipeline.md)에 정리했다.
...
`.modelContainer(_:)`가 environment의 `modelContext`까지 채운다는 근거와 `@Query`가 그 context를 쓰는 구조는 [Preview의 mock container 문서](./133-swiftdata-preview-mock-container.md)에 정리했다.
...
어느 쪽이 옳다기보다 **격리를 원하는지 공유를 원하는지**로 고른다. `static var`와 `static let`의 차이와 같은 구도이며, 자세한 실측은 [`static var { }`와 `static let = []`](./016-static-stored-vs-computed-property.md)에 있다.
