...
`Hashable`과 해시 충돌은 [별도 문서](./026-hashable-id-and-collisions.md)에, `ForEach`의 `id` 지정은 [여기](./053-foreach-id-and-identity-keypath.md)에 정리했다. 이 문서는 **id 중복 시의 실제 동작**을 다룬다.
...
[static var vs static let 문서](./016-static-stored-vs-computed-property.md)에서 chapter-57의 같은 문제를 다뤘다. 해결도 같다.
...
- 저장 프로퍼티가 하나 줄어든다
- [`Codable`에서 `CodingKeys` 문제](./117-codable-and-codingkey.md)도 사라진다
...
[ForEach의 id와 key path 문서](./053-foreach-id-and-identity-keypath.md)에서 다룬 내용이다. 중복 가능성이 있는 컬렉션에는 `\.self`를 쓰지 않는다.
