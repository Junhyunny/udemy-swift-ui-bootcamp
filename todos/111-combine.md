...
구독 수명 관리는 [별도 문서](./112-combine-cancellable-and-store.md)에, 연산자는 [여기](./113-combine-operators.md)에, async/await와의 비교는 [여기](./114-combine-vs-async-await.md)에 정리했다. 이 문서는 **전체 그림과 `AnyPublisher`** 를 다룬다.
...
**`Failure`가 `Never`면 절대 실패하지 않는다는 뜻**이고, 그때만 인자 하나짜리 `sink`를 쓸 수 있다. [연산자 문서](./113-combine-operators.md)에서 다룬 `replaceError`가 이 타입을 바꾼다.
...
**다만 이 방식은 구형이다.** [`@Observable`](./050-observation-framework-and-observable.md)이 iOS 17에서 이를 대체했고, 그쪽은 Combine 의존이 없다. 그래서 최신 SwiftUI 코드에서 Combine을 쓸 이유가 줄었다.
...
[`AnyCancellable`도 클래스](./112-combine-cancellable-and-store.md)였던 것과 같은 구조다. `Any` 접두사가 붙은 Combine 타입들은 대체로 타입 소거 래퍼다.
...
[`some View`와 `ModifiedContent`](./034-some-keyword-opaque-types.md)에서 본 것과 똑같은 문제다.
...
`some`이 성능상 유리하지만, **Combine 생태계에서는 `AnyPublisher`가 사실상 표준**이다. 기존 코드와의 일관성, 프로퍼티로 저장할 수 있다는 점 때문이다. [`some`과 `any`의 차이](./034-some-keyword-opaque-types.md)에서 다룬 트레이드오프와 같은 구도다.
...
**다만 `url!` 강제 언래핑이 위험하다.** `Endpoint.url`이 `URL?`이므로 `nil`이면 크래시한다. [URLComponents 문서](./119-urlcomponents-and-url-building.md)에서 다룬 문제이고, 오류를 publisher로 흘리는 편이 안전하다.
