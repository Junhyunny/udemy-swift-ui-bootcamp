...
`presentationMode`와 `dismiss`의 비교는 [별도 문서](./079-presentation-mode-vs-dismiss.md)에 정리했다. 이 문서는 **`sheet`의 클로저와 결과 전달**을 다룬다.
...
[클로저와 trailing closure 문법](./040-closures-and-view-builders.md)에서 다룬 multiple trailing closure 규칙이다. 첫 번째만 라벨을 생략할 수 있고 나머지는 라벨을 붙인다.
...
[`@Binding`](./044-property-wrapper-dollar-sign.md)으로 시트가 부모의 상태를 쓰게 한다.
...
**시트가 열릴 때 실행할 곳은 따로 없다.** `content` 안의 뷰에서 `.onAppear`나 [`.task`](./103-task-modifier-and-async-lifecycle.md)를 쓴다.
