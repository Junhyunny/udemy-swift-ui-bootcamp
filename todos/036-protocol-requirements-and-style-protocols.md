...
protocol과 상속의 관계는 [Swift의 타입 체계와 상속 구조](./020-swift-type-system-and-inheritance.md)에, `@ViewBuilder`는 [`@ViewBuilder` 함수 vs 별도 `View` 구조체](./041-viewbuilder-vs-view-struct.md)에 있다. 이 문서는 **`LabelStyle`을 구현할 때 무슨 일이 일어나는지**를 다룬다.
