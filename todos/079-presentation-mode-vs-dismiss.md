...
`@Environment` 전반과 `\.dismiss`의 용법은 [별도 문서](./048-environment-property-wrapper.md)에 정리했다. 이 문서는 **두 API의 비교**를 다룬다.
...
SwiftUI 첫 버전부터 있던 API이고, iOS 15에서 더 나은 대체제가 나왔다. [`NavigationView` → `NavigationStack` 전환](./073-navigation-stack-vs-navigation-view.md)과 비슷한 흐름이다.
...
`dismiss()`처럼 변수를 함수로 호출할 수 있는 것은 `callAsFunction` 덕분이다. [`@Environment` 문서](./048-environment-property-wrapper.md)에서 다룬 `openURL`과 같은 방식이다.
...
[`@Environment`의 Actions 범주](./048-environment-property-wrapper.md)가 이때 정착한 패턴이다. `openURL`, `refresh`, `openWindow`도 같은 형태다.
...
배포 타겟이 iOS 15 이상이면 바로 바꿀 수 있다. [버전 확인 방법](./074-swift-ios-device-compatibility.md)은 별도 문서에 정리했다.
...
**닫으려는 뷰 안에서 선언해야 한다.** [`@Environment` 문서](./048-environment-property-wrapper.md)에서 Apple 문서를 인용해 자세히 다뤘다.
...
`CourseDetailView`처럼 여러 곳에서 재사용될 수 있는 화면은 **자기가 어떤 스택에 올라와 있는지 몰라도 되는** `dismiss()`가 맞다. [NavigationPath 문서](./075-navigation-path-and-typed-array.md)에서 비교했다.
...
- `NavigationView` → `NavigationStack` ([관련 문서](./073-navigation-stack-vs-navigation-view.md))
- `ZStack` + 투명 `NavigationLink` 패턴 → 라벨에 직접 넣기 ([히트 테스트 문서](./084-swiftui-hit-testing-vs-dom-events.md))

`CartView.swift`의 `@ObservedObject`도 [`@Observable`](./050-observation-framework-and-observable.md)로 옮길 수 있는 구형 API다.
