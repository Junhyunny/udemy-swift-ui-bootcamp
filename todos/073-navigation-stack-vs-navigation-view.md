...
`NavigationStack`의 기본 역할과 `navigationTitle`을 붙이는 위치는 [별도 문서](./072-navigation-stack-and-title.md)에 정리했다. 이 문서는 **구형 API와의 관계**를 다룬다.
...
[`ImageResource` 문서](./058-image-resource-and-asset-symbols.md)에서도 나온 구분이다. SwiftUI 타입의 등장 시점은 **Swift 언어 버전이 아니라 OS·SDK 버전**으로 정해진다.
...
`NavigationView`는 SwiftUI 첫 버전부터 있었고, 3년 뒤 iOS 16에서 `NavigationStack`과 `NavigationSplitView` 둘로 나뉘어 대체됐다. 버전 확인 방법 전반은 [호환성 자료 문서](./074-swift-ios-device-compatibility.md)에 정리했다.
...
링크는 **값만** 넘기고, 그 값을 어떤 뷰로 그릴지는 `navigationDestination`이 한 번만 선언한다. 이 연결 원리는 [별도 문서](./076-navigation-link-value-and-destination.md)에 정리했다.
...
마지막 문장이 이 기능의 존재 이유를 요약한다. [chapter-51의 딥링크 예제](./085-deep-link-and-url-scheme.md)에서 `NavigationCoordinator`가 `path.append(Route.test)`를 하던 것이 정확히 이 용법이다. `NavigationView`로는 이런 코드를 쓸 수 없었다.
...
**② 타입이 `Hashable`이어야 한다.** 값 기반이므로 넘길 타입에 `Hashable` 준수가 필요하다. [enum과 Hashable 문서](./028-enum-hashable-conformance.md) 참조.
...
**이름만 바꿔도 동작한다.** `NavigationLink(destination:)` 방식은 [여전히 현행 API](./078-navigation-link-two-styles-mixed.md)이므로 함께 고칠 필요가 없다.
...
**`ZStack` + 투명 `NavigationLink` 패턴도 함께 정리할 수 있다.** 이건 `NavigationView` 시절의 우회책이었다. [히트 테스트 문서](./084-swiftui-hit-testing-vs-dom-events.md)에서 다룬 대로, 현행 API에서는 라벨에 직접 넣으면 된다.
...
이러면 `path`로 코드에서 화면을 제어할 수 있고, 목적지가 탭할 때 생성된다. 다만 `Course`가 `Hashable`을 채택해야 하고([enum과 Hashable](./028-enum-hashable-conformance.md) 참조), [`id`가 안정적이어야 한다](./054-duplicate-id-in-list.md)는 전제가 붙는다. 현재 `Course.sample`은 계산 프로퍼티라 그 전제가 깨져 있다.
