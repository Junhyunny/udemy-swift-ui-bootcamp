...
Combine 전반은 [별도 문서](./111-combine.md)에, 연산자는 [여기](./113-combine-operators.md)에 정리했다. 이 문서는 **`onReceive`와 실무 활용**을 다룬다.
...
**`P.Failure == Never` 제약**에 주목할 만하다. **실패할 수 있는 publisher는 받지 않는다.** 오류를 처리할 방법이 없으므로 [`replaceError`나 `catch`](./113-combine-operators.md)로 미리 없애야 한다.
...
[구독 수명 관리](./112-combine-cancellable-and-store.md)에서 다룬 번거로움이 사라진다. **뷰 안에서 publisher를 쓸 때는 `onReceive`가 기본 선택지다.**
...
[`.task`](./103-task-modifier-and-async-lifecycle.md)가 뷰 생명주기에 묶이므로 취소가 자동이다. Combine 의존이 사라지는 것도 이점이다.
...
[`NotificationCenter`](./109-notification-center.md)의 이벤트를 받는다.
...
타이핑할 때마다 요청하는 대신 **400ms 동안 입력이 멈추면 한 번만** 요청한다. `onChange`만으로는 구현하기 번거로운 부분이다. [Combine과 async/await 비교](./114-combine-vs-async-await.md)에서 다룬 Combine의 강점이다.
...
| 목적 | `onReceive` 대신 |
| --- | --- |
| 앱 상태 변화 | [`@Environment(\.scenePhase)`](./048-environment-property-wrapper.md) |
| 값 변화 감지 | [`onChange(of:)`](./052-onchange-old-new-value.md) |
| 비동기 작업 | [`.task`](./103-task-modifier-and-async-lifecycle.md) |
| 주기적 화면 갱신 | `TimelineView` |
| `@Observable` 객체 관찰 | 그냥 프로퍼티를 읽으면 된다 |

**`@Observable`을 쓰면 `onReceive`가 필요 없어지는 경우가 많다.** [Observation 문서](./050-observation-framework-and-observable.md)에서 다룬 대로 프로퍼티를 읽는 것만으로 의존성이 등록된다.
