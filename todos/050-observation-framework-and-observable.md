...
환경 주입은 [`@Environment` 문서](./048-environment-property-wrapper.md)에 정리했다.
...
**`internal import Combine`이 사라지는 것도 이점**이다. `ObservableObject`와 `@Published`가 Combine 소속이라 필요했던 import다. [Combine 문서](./111-combine.md) 참조.
...
`@Observable` 객체의 프로퍼티에 `$`로 바인딩을 만들려면 `@Bindable`이 필요하다. [프로퍼티 래퍼의 `$`](./044-property-wrapper-dollar-sign.md) 참조.
...
지금은 계층이 얕아 ①로 충분하다. [`@Environment` 문서](./048-environment-property-wrapper.md)에서 선택 기준을 다뤘다.
