...
`NavigationStack`이 `NavigationView`를 어떻게 대체했는지는 [별도 문서](./073-navigation-stack-vs-navigation-view.md)에 정리했다. 이 문서는 **구형 `NavigationLink` 이니셜라이저 두 개**를 다룬다.
...
`path` 배열 하나가 이 문제를 모두 해결한다. [`NavigationPath`와 타입 배열 문서](./075-navigation-path-and-typed-array.md) 참조.
...
값과 목적지가 연결되는 원리는 [별도 문서](./076-navigation-link-value-and-destination.md)에 정리했다.
...
버전 확인 방법은 [호환성 문서](./074-swift-ios-device-compatibility.md)에 정리했다.
...
`\.dismiss`가 `NavigationStack`에서 **pop 역할**을 한다. 자세한 내용은 [`@Environment` 문서](./048-environment-property-wrapper.md)에 정리했다.
