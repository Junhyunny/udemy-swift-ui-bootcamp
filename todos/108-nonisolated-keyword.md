...
`@MainActor`가 타입에 붙으면 **그 안의 모든 메서드와 프로퍼티 접근이 메인 액터 소속**이 된다. 액터와 `@MainActor`의 기본은 [`actor` 타입 문서](./107-swift-actor-type.md)와 [`MainActor`와 iOS 스레드 모델 문서](./106-main-actor-and-ios-threading.md)에 정리되어 있다.
...
참고로 `@Animatable`이 합성하는 `animatableData`에도 **매크로가 `nonisolated`를 자동으로 붙여 준다.** 같은 이유(`Animatable` 요구사항이 `nonisolated`)이고, 그래서 그쪽은 손댈 일이 없다. [`@Animatable` 매크로 문서](./099-animatable-macro.md)의 확장 결과에서 확인할 수 있다.
