...
보간의 원리 자체는 [`animatableData` 문서](./animatable-data-and-interpolation.md)에 정리되어 있다. 이 문서는 **매크로가 무엇을 만들어 주는지**를 다룬다.
...
매크로가 언제 어떻게 코드를 끼워 넣는지는 [Swift macro와 빌드 파이프라인 문서](./swift-macros-and-build-pipeline.md)에 정리되어 있다.
...
- **`nonisolated`가 붙어 있다** — 매크로가 알아서 붙여 준다. 이 키워드의 의미는 [`nonisolated` 문서](./nonisolated-keyword.md)에 정리되어 있다.
...
`Bool`은 `true`와 `false` 사이에 값이 없으므로 두 번째 길은 애초에 불가능하다. 자세한 이유는 [`animatableData` 문서](./animatable-data-and-interpolation.md)의 "그래서 `Bool`은 왜 안 되나"에 있다.
...
`@Animatable`은 iOS 26부터다. 이 프로젝트의 배포 타깃은 `IPHONEOS_DEPLOYMENT_TARGET = 26.5`라 문제가 없지만, **더 낮은 버전을 지원해야 하면 `AnimatablePair`로 손수 작성**해야 한다. [Swift·iOS 버전 호환성 문서](./swift-ios-device-compatibility.md)와 이어진다.
...
공식 문서의 표현으로는 "does not correspond one-to-one with a stored property"인 경우다. 예제는 [`animatableData` 문서](./animatable-data-and-interpolation.md)의 "직접 쓰는 것이 더 나은 경우"에 있다.
...
이 프로젝트는 빌드 설정이 `SWIFT_DEFAULT_ACTOR_ISOLATION = MainActor`라 **아무것도 안 붙여도 이미 `@MainActor`**다. 즉 이 줄은 중복이고, 오히려 `path(in:)`에 `nonisolated`를 붙여야 하는 원인이 된다. 자세한 내용은 [`nonisolated` 문서](./nonisolated-keyword.md)에 정리했다.
