...
`\.self` 형태도 또 다른 것이다. `ForEach(0..<n, id: \.self)`의 `\.self`는 **key path**이며, 값 자신을 가리키는 경로다. 이 얘기는 [ForEach의 id와 key path](./053-foreach-id-and-identity-keypath.md)에 있다.
...
마지막 형태가 [implicit member expression](./015-static-type-properties-and-implicit-init.md)이다. 반환 타입이 `ExchangeRate`로 적혀 있으므로 타입을 생략할 수 있다.
...
`Self`가 없다면 프로토콜에서 "구현하는 타입 자신"을 표현할 방법이 없다. [`Hashable`의 `==`](./027-hash-into-and-java-comparison.md) 시그니처에도 `Self`가 등장한다.
...
`struct`는 상속이 없어 둘이 항상 같지만, `class`에서는 갈린다. [struct와 class](./019-struct-vs-class.md)의 차이가 여기서도 드러난다.
