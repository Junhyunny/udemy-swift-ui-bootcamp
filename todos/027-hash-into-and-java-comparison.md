...
`hash(into:)`에는 `override`를 붙이지 않는다. `struct`는 상속이 없으므로 애초에 `override`가 불가능하다. [Swift의 타입 체계와 상속](./020-swift-type-system-and-inheritance.md)에서 다룬 구분이다.
...
무작위 시드를 쓰기 때문이다. 이건 해시 충돌 공격(HashDoS)을 막는 보안 조치이기도 하다. [해시 충돌 문서](./026-hashable-id-and-collisions.md)에서 다룬 "해시 값을 저장하면 안 된다"가 이 이유다.
...
`lastSeen`이 갱신될 때마다 다른 값이 되면 `List`의 행 신원이 흔들린다. [static 프로퍼티 문서](./016-static-stored-vs-computed-property.md)에서 본 문제와 같은 계열이다.
...
`Hashable`이 필요한 이유는 `navigationDestination(for:)`의 `where D : Hashable` 제약이다. [값과 목적지 연결 문서](./076-navigation-link-value-and-destination.md) 참조.

**여기에 `static var sample`(계산 프로퍼티) + `UUID()`가 겹친 문제가 있다.** `sample`을 읽을 때마다 새 `UUID`가 발급되므로 `id`가 매번 바뀐다. 자동 합성된 `==`가 `id`를 비교하니 `path`에 넣은 값과 `List`가 그리는 값이 일치하지 않을 수 있다. [static var vs static let 문서](./016-static-stored-vs-computed-property.md)에서 다룬 chapter-57과 같은 상황이다.
