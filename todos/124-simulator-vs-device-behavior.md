...
**왜 필요한가.** [`Timer.publish`](./110-timer-publisher-and-onreceive.md)는 앱이 백그라운드로 가면 **멈춘다.** 앱이 서스펜드되면 런루프가 돌지 않기 때문이다.
...
[알림 문서](./135-user-notifications-framework.md)에서 다룬 `UNTimeIntervalNotificationTrigger`도 시뮬레이터와 실기기가 다르다.
...
[`UIViewRepresentable` 문서](./087-uiviewrepresentable-and-uikit-bridge.md)에서 플랫폼별 분기에 `#if os(iOS)`를 쓴 것과 같은 문법이다.
...
`remainingSeconds`가 계산 프로퍼티인 것이 핵심이다. [계산 프로퍼티 문서](./010-computed-property-with-closure-body.md)에서 다룬 "접근할 때마다 계산"이 여기서는 장점이 된다.

**③ `leftTime`이 IUO인 문제도 함께 사라진다**

[암시적 언래핑 옵셔널 문서](./005-implicitly-unwrapped-optional.md)에서 지적한 `Date!`가 `Date?`(또는 `endDate`)로 정리된다.
...
| **Core Haptics** | **지원 안 함** — [chapter-51에서 겪은 로그](./085-deep-link-and-url-scheme.md)의 원인 |
