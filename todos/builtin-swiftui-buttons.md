...
`List(selection:)`은 각 행의 **identity 값**을 모은다. [`Identifiable` 프로토콜 문서](./identifiable-protocol.md), [`id`에 쓰이는 `Hashable` 문서](./hashable-id-and-collisions.md)와 이어진다.
...
`editMode`는 `Binding<EditMode>?`라 **옵셔널이고 두 번 벗겨야** 한다. `EditMode`에는 `.inactive`, `.transient`, `.active` 세 상태가 있고 `isEditing` 계산 프로퍼티가 있다. [`@Environment` 문서](./environment-property-wrapper.md)와 이어진다.
...
`Button { openURL(url) }`과 비슷하지만, **링크라는 의미가 접근성에 전달**되고 시스템이 적절히 처리한다. 딥링크·커스텀 스킴도 열 수 있다. [딥링크와 URL 스킴 문서](./deep-link-and-url-scheme.md)와 이어진다.
...
`makeBody`가 override가 아니라 프로토콜 요구사항이라는 점은 [style 프로토콜 문서](./protocol-requirements-and-style-protocols.md)에 정리되어 있다.
...
배치와 툴바 API 전반은 [`toolbar` API 문서](./toolbar-api-use-cases.md)에 정리되어 있다.
