...
`body`에 `@ViewBuilder`가 붙어 있어서 `if`/`switch`로 분기하는 것도 가능하다. `@ViewBuilder`의 원리는 [클로저와 view builder](./040-closures-and-view-builders.md)에 있다.
...
`extension`으로 `View`에 메서드를 붙이는 것이 왜 되는지는 [extension 문서](./014-extension-keyword.md)에서 다룬다.
...
- **내용을 새로 구성**하는 것이 목적이라면 별도 `View` 구조체가 맞다. `ViewModifier`는 받은 뷰를 감싸는 용도다. 이 판단 기준은 [ViewBuilder 함수 vs View 구조체](./041-viewbuilder-vs-view-struct.md)와 같은 결이다.
...
`body`는 렌더링마다 호출된다. `GeometryReader`처럼 비용이 있는 것을 넣을 때는 주의한다. [GeometryReader와 성능](./068-geometry-reader-performance.md)에 정리했다.
